                ...
                try:
                    ...
                    if boundary:
                        ...
                        for i, part in enumerate(parts):
                            ...
                            # 计算图片数量（兼容iOS的files字段名和标准的image_N字段名）
                            elif 'Content-Type: image/' in part_str:
                                # 检查是否包含图片相关的字段名
                                if ('name="files"' in part_str or 
                                    'name="image_' in part_str or
                                    'filename=' in part_str):
                                    image_count += 1
                                    print(f"📸 发现图片文件，当前计数: {image_count}")
